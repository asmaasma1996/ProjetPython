...
class Board:
    ...
    def get_valid_moves(self, piece):
        ...
        if piece.color == RED or piece.king:
            moves.update(self._travers_left(row -1, max(row-3, -1), -1, piece.color, left))
            moves.update(self._travers_right(row -1, max(row-3, -1), -1, piece.color, right))
        if piece.color == WHITE or piece.king:
            moves.update(self._travers_left(row +1, min(row+3, ROWS), 1, piece.color, left))
            moves.update(self._travers_right(row +1, min(row+3, ROWS), 1, piece.color, right))
